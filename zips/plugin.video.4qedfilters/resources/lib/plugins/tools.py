...
def password_handler(url):
    ...
    if time.time() > expires_at or expires_at == '':
        input = ''
        if not COLOR1 == "":
            enterpass = "[COLOR %s]Are you over the age of 18?[/COLOR]" % COLOR1
        else:
            enterpass = "Are you over the age of 18?"
        ...
        if input == dec_pass:
            ...
            if not COLOR2 == "":
                passfail = "[COLOR %s]Wrong Answer... Unfortunately you won't be able to watch any porn today![/COLOR]" % COLOR2
            else:
                passfail = "Wrong Answer... Unfortunately you won't be able to watch any porn today!"
            prot_xml += "<dir>"\
                    "    <title>%s</title>"\
                    "    <thumbnail>https://d2v9y0dukr6mq2.cloudfront.net/video/thumbnail/63y00O5/access-denied-message-on-a-computer-screen_vnefbpypvx__F0000.png</thumbnail>"\
                    "</dir>" % passfail
    else:
        if 'http' in xml_loc:
            prot_xml = requests.get(xml_loc).content
    ...
    try:
        ...
        if 'pairlist' in url:
            ...
            for item in PAIR_LIST:
                the_title = '[COLOR %s]Easy to Pair  -  [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, item[0].capitalize())
                names.append(the_title)
            selected = xbmcgui.Dialog().select('[COLOR %s]4QED FILTERS - Select Site To Pair Device With[/COLOR]' % COLOR2, names)
    ...
    try:
        ...
        if 'authlist' in url:
            ...
            for item in AUTH_LIST:
                the_title = '[COLOR %s]Authorize  -  [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, item[0].capitalize())
                names.append(the_title)
            selected = xbmcgui.Dialog().select('[COLOR %s]4QED FILTERS - Select Debrid Service To Authorize With[/COLOR]' % COLOR2, names)
