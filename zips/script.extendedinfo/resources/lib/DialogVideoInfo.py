	...
	class DialogVideoInfo(DialogBaseInfo, window_type):
		...
		def play_movie(self):
			if self.dbid:
				...
				dbid = 0
				url = 'plugin://plugin.video.metalliq-forqed/movies/play/tmdb/%s' % self.info.get('id', '')
		...
		def add_movie_to_library(self):
			if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Add [B]%s[/B] to library?' % self.info['title']):
		...
		def remove_movie_from_library(self):
			if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Remove [B]%s[/B] from library?' % self.info['title']):
