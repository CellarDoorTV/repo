...
def get_tmdb_window(window_type):
	...
	class DialogVideoList(DialogBaseList, window_type):
		...
		def context_menu(self):
			...
			if selection == 0:
				if self.listitem.getProperty('TVShowTitle'):
					url = 'plugin://plugin.video.metalliq-forqed/tv/play/%s/1/1' % tvdb_id
					...
					if self.listitem.getProperty('dbid'):
						...
						dbid = 0
						url = 'plugin://plugin.video.metalliq-forqed/movies/play/tmdb/%s' % item_id
					PLAYER.play_from_button(url, listitem=None, window=self, type='movieid', dbid=dbid)
			if selection == 1:
				if self.listitem.getProperty('TVShowTitle'):
					TVLibrary = xbmcaddon.Addon('plugin.video.metalliq-forqed').getSetting('tv_library_folder')
					if self.listitem.getProperty('dbid'):
						...
						if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Add [B]%s[/B] to library?' % self.listitem.getProperty('TVShowTitle')):
					...
					if self.listitem.getProperty('dbid'):
						if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Remove [B]%s[/B] from library?' % self.listitem.getProperty('title')):
							Utils.get_kodi_json(method='VideoLibrary.RemoveMovie', params='{"movieid": %s}' % dbid)
							MovieLibrary = xbmcaddon.Addon('plugin.video.metalliq-forqed').getSetting('movies_library_folder')
						...
						if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Add [B]%s[/B] to library?' % self.listitem.getProperty('title')):
