                ...
                if self.type == "tv" or self.type == "episode":
                    if self.listitem.getProperty("dbid"):
                        ...
                        if os.path.exists(show_playlist):
                            url = show_playlist
                        else: url = 'plugin://plugin.video.metalliq-forqed/tv/play/%s/1/1/library' % (tvdb_id)
                    else: url = 'plugin://plugin.video.metalliq-forqed/tv/play/%s/1/1/library' % (tvdb_id)
                else:
                    if self.listitem.getProperty("dbid"): url = 'temp'
                    else: url = 'plugin://plugin.video.metalliq-forqed/movies/play/tmdb/%s/library' % (item_id)
                PLAYER.qlickplay(url, listitem=None, dbid=dbid, window=self)
            elif selection == 1:
                if self.type == "tv" or self.type == "episode":
                    TVLibrary   = METALLIQ4QED.getSetting("tv_library_folder")
                    TVPlaylists = METALLIQ4QED.getSetting("tv_playlist_folder")
                    if self.listitem.getProperty("dbid"):
                    ...
                    MovieLibrary = METALLIQ4QED.getSetting("movies_library_folder")
                    if self.listitem.getProperty("dbid"):
