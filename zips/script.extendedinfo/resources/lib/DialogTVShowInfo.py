...
def get_tvshow_window(window_type):
	...
	class DialogTVShowInfo(DialogBaseInfo, window_type):
		...
		def browse_tvshow(self):
			url = 'plugin://plugin.video.metalliq-forqed/tv/tvdb/%s/' % self.info['tvdb_id']
			self.close()
		...
		def show_manage_dialog(self):
			manage_list = []
			manage_list.append(["[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed Settings", 'Addon.OpenSettings("script.extendedinfo")'])
			manage_list.append(["[COLOR limegreen]M[/COLOR]etalli[COLOR limegreen]Q[/COLOR] 4[COLOR limegreen]Q[/COLOR]ed Settings", 'Addon.OpenSettings("plugin.video.metalliq-forqed")'])
		...
		def play_tvshow(self):
			url = 'plugin://plugin.video.metalliq-forqed/tv/play/%s/1/1' % self.info['tvdb_id']
			PLAYER.play_from_button(url, listitem=None, window=self, dbid=0)

		@ch.click(20)
		def add_tvshow_to_library(self):
			if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Add [B]%s[/B] to library?' % self.info['TVShowTitle']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.metalliq-forqed/tv/add_to_library/%s)' % self.info['tvdb_id'])
		...
		def remove_tvshow_from_library(self):
			if xbmcgui.Dialog().yesno('[COLOR limegreen]EIM[/COLOR]4[COLOR limegreen]Q[/COLOR]ed', 'Remove [B]%s[/B] from library?' % self.info['TVShowTitle']):
				if os.path.exists(xbmc.translatePath('%s%s/' % (Utils.METALLIQ4QED_TV_FOLDER, self.info['tvdb_id']))):
					Utils.get_kodi_json(method='VideoLibrary.RemoveTVShow', params='{"tvshowid": %s}' % int(self.info['dbid']))
					shutil.rmtree(xbmc.translatePath('%s%s/' % (Utils.METALLIQ4QED_TV_FOLDER, self.info['tvdb_id'])))
					Utils.after_add(type='tv')
