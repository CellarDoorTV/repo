...
def update_library():
	...
	for id in shows:
		...
		updated += 1
	if clean_needed:
		plugin.setProperty('plugin.video.metalliq-forqed.clean_library', 'true')
	if updated > 0:
...
def setup_library(library_folder):
	...
	if not xbmcvfs.exists(library_folder):
		...
		msg = 'Would you like to automatically set [COLOR limegreen]M[/COLOR]etalli[COLOR limegreen]Q[/COLOR] 4[COLOR limegreen]Q[/COLOR]ed as a tv shows source?'
		if plugin.yesno('Library setup', msg):
			try:
				source_thumbnail = plugin.get_media_icon('tv')
				source_name = '[COLOR limegreen]M[/COLOR]etalli[COLOR limegreen]Q[/COLOR] 4[COLOR limegreen]Q[/COLOR]ed TV shows'
