...
def update_players_local(zip_path):
	extract_to = xbmc.translatePath('special://profile/addon_data/plugin.video.metalliq-forqed/Players')  
...
def flat_extract(z, extract_to, members=None):
	...
	if not os.path.exists(extract_to):
		...
		if xbmc.getInfoLabel('Window(home).Property(running)') == 'totalmetalliq-forqed':
			...
			if plugin.yesno('[COLOR limegreen]M[/COLOR]etalli[COLOR limegreen]Q[/COLOR] 4[COLOR limegreen]Q[/COLOR]ed: Update players', 'Do you want to remove existing players first?'):
				empty_folder(extract_to)
